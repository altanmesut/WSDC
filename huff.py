...
def huffman_encode(metin, dil):
    ...
    size = c_int(len(metin)*25)
    lang = c_int(diller.index(dil))
    output = c_char_p(functions.huffmanEncode(text, size, lang))
    return ConstBitStream(bin=output.value.decode(encoding='utf8', errors='ignore')).tobytes()

def huffman_decode(veri, dil):
    data = c_char_p(bytes(ConstBitStream(veri).bin, encoding='utf8'))
